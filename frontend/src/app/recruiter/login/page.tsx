...
export default function RecruiterLogin() {
  ...
  useEffect(() => {
    if (isAuthenticated && user && !isLoading) {
      ...
      const timer = setTimeout(() => {
        ...
        // Prioritize recruiter portal for recruiter users
        if (role === 'recruiter') {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      console.log('[Recruiter Login Page] Attempting login...');
      await login(email, password);
      console.log('[Recruiter Login Page] Login successful, redirecting...');
      ...
      console.log('[Recruiter Login Page] User role:', role);
      
      // Redirect based on role - prioritize recruiter portal for recruiter users
      if (role === 'recruiter') {
        router.push('/recruiter/dashboard');
      } else if (role === 'admin') {
        router.push('/company/dashboard'); // Admin can choose
      ...
      console.error('[Recruiter Login Page] Login error:', err);
      console.error('[Recruiter Login Page] Error details:', {
        message: err.message,
        stack: err.stack,
        name: err.name,
      ...
      <div className="max-w-md mx-auto px-6 py-16">
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-full bg-barnes-violet flex items-center justify-center mx-auto mb-4">
              <span className="text-white font-bold text-2xl">R</span>
            </div>
            <h1 className="text-3xl font-bold text-barnes-dark-violet mb-2">Recruiter Login</h1>
            <p className="text-barnes-dark-gray">Access your recruiter dashboard</p>
          </div>
